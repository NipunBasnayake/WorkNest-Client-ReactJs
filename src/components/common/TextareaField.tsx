...
export const TextareaField = forwardRef<HTMLTextAreaElement, TextareaFieldProps>(
  ({ label, error, hint, id, className = "", rows = 4, required, ...rest }, ref) => {
    ...
    const baseTextareaClasses = `w-full rounded-xl px-4 py-2.5 text-sm border transition-all duration-200
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          className={`${baseTextareaClasses} ${error ? "border-red-400 focus:ring-red-400/60 focus:border-red-400" : ""}`}
          style={{
        ...
        {error && (
          <span id={`${textareaId}-error`} className="text-xs text-red-500 mt-0.5 flex items-center gap-1">
