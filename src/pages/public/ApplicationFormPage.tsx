  ...
  return <PageContainer className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
      <form className="space-y-6" onSubmit={submit}>
        <div><p className="text-sm font-semibold text-purple-600">Apply to {job.company.companyName}</p><h1 className="mt-2 text-3xl font-bold tracking-tight" style={{ color: "var(--text-primary)" }}>{job.title}</h1><p className="mt-2 text-sm" style={{ color: "var(--text-secondary)" }}>Complete the form below. No account or login is required.</p></div>
...
function Summary({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex gap-3"><span className="text-purple-600">{icon}</span><div><p className="text-xs" style={{ color: "var(--text-tertiary)" }}>{label}</p><p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{value}</p></div></div>; }
