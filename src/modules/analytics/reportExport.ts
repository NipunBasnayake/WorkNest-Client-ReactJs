...
const csvCell = (value: ReportCell) => { const quote = String.fromCharCode(34); return quote + clean(value).replaceAll(quote, quote + quote) + quote; };
...
export function exportExcel(report: ReportDataset) {
  const table = `<table><thead><tr>${report.headers.map((item) => `<th>${clean(item)}</th>`).join('')}</tr></thead><tbody>${report.rows.map((row) => `<tr>${row.map((item) => `<td>${clean(item)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
  download(`\ufeff<html><body><h1>${report.title}</h1>${table}</body></html>`, 'application/vnd.ms-excel', `${report.title}.xls`);
}
