...
import { deriveFormalReport, getFormalReportStatusOptions, getReportCatalog, loadFormalReport, loadFullFormalReport, reportToDataset, type FormalReportData, type FormalReportDefinition, type FormalReportId, type FormalReportRequest } from '@/modules/analytics/formalReports';
...
function ReportCatalog({ catalog }: { catalog: FormalReportDefinition[] }) {
  return <nav aria-label='Available reports' className='grid grid-cols-1 gap-4 pb-8 sm:grid-cols-2 lg:grid-cols-4'>
    {catalog.map((report) => {
      const Icon = reportIcons[report.id];
      return <Button key={report.id} to={`${tenantRoutes.reports()}/${report.id}`} variant='secondary' className='group h-auto min-h-40 w-full items-stretch justify-start overflow-hidden rounded-2xl border-[var(--border-default)] p-0 text-left transition duration-200 hover:-translate-y-0.5 hover:border-primary-400/60 hover:shadow-lg'>
...
function ReportDetail({ active, role }: { active: FormalReportDefinition; role: NormalizedAppRole }) {
  ...
  const reportView = useMemo(() => !query.data || active.serverPaginated ? query.data : deriveFormalReport(active.id, query.data, rows), [active.id, active.serverPaginated, query.data, rows]);
  ...
  return <main className='min-w-0 space-y-5 pb-8'>
    <PageHeader title={active.title} description={active.description} secondaryActions={<Button variant='outline' size='sm' onClick={() => void query.refetch()} loading={query.isFetching}><RefreshCw size={14} />Refresh</Button>} />
    <ReportFilters reportId={active.id} filters={filters} setFilter={handleReportFilter} onReset={() => { reset(); setSearch(''); setColumnFilters({}); setSort(null); setServerPage(1); }} />
    ...
    {query.data && reportView && <><ReportKpis report={reportView} recordCount={totalItems} /><ReportCharts report={reportView} /><SectionCard title='Search records' subtitle='Search across every displayed business column. Column-specific filters are available directly under the table headers.' variant='dense' className='print:hidden'><SearchField label='Search report records' value={search} onChange={(event) => { setServerPage(1); setSearch(event.target.value); }} onClear={() => { setServerPage(1); setSearch(''); }} placeholder='Search names, statuses, departments, identifiers…' /></SectionCard><AdvancedReportTable columns={query.data.columns} rows={rows} visibleKeys={new Set(query.data.columns.filter((column) => !hiddenColumns.has(column.key)).map((column) => column.key))} onToggleColumn={(key) => setHiddenColumns((current) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; })} columnFilters={columnFilters} onColumnFilter={handleColumnFilter} sort={sort} onSort={handleSort} pageOffset={pageOffset} pageSize={pageSize} /><ReportSummary report={query.data} filteredCount={totalItems} visibleColumns={query.data.columns.length - hiddenColumns.size} /><ReportExports report={query.data} currentRows={currentRows} allRows={rows} fullReportLoader={fullReportLoader} /><Pagination currentPage={currentPage} totalItems={totalItems} pageSize={pageSize} onPageChange={active.serverPaginated ? setServerPage : reportPagination.setCurrentPage} onPageSizeChange={active.serverPaginated ? (size) => { setServerPageSize(size); setServerPage(1); } : reportPagination.setPageSize} className='rounded-2xl border print:hidden' /></>}
  </main>;
}

function ReportFilters({ reportId, filters, setFilter, onReset }: { reportId: FormalReportId; filters: ReturnType<typeof useAnalyticsFilterStore.getState>['filters']; setFilter: ReturnType<typeof useAnalyticsFilterStore.getState>['setFilter']; onReset: () => void }) {
  const statuses = getFormalReportStatusOptions(reportId);
  const showDepartment = ['employees', 'new-joiners', 'employee-status', 'attendance', 'leave', 'departments', 'recruitment-jobs', 'recruitment-applications', 'recruitment-interviews', 'recruitment-hiring', 'notifications'].includes(reportId);
  const showEmployee = ['employees', 'new-joiners', 'employee-status', 'attendance', 'leave', 'projects', 'tasks', 'workload', 'audit', 'recruitment-interviews', 'notifications'].includes(reportId);
  const showProject = ['projects', 'tasks', 'project-progress', 'workload', 'notifications'].includes(reportId);
  const showTeam = ['teams', 'projects', 'tasks', 'project-progress', 'workload', 'notifications'].includes(reportId);
  const textFilter = (key: 'department' | 'employeeId' | 'projectId' | 'teamId', label: string) => <label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>{label}</span><input value={filters[key]} onChange={(event) => setFilter(key, event.target.value)} placeholder={`All ${label.toLowerCase()}s`} className='h-10 w-full rounded-xl border px-3 text-sm' style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-default)' }} /></label>;
  return <SectionCard title='Report filters' subtitle='Filters define the governed record set and are applied before summaries, charts, tables, and exports.' action={<Button variant='ghost' size='sm' onClick={onReset}>Reset filters</Button>} variant='dense' className='print:hidden'><div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6'><label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>From date</span><input type='date' value={filters.fromDate} onChange={(event) => setFilter('fromDate', event.target.value)} className='h-10 w-full rounded-xl border px-3 text-sm' style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-default)' }} /></label><label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>To date</span><input type='date' value={filters.toDate} onChange={(event) => setFilter('toDate', event.target.value)} className='h-10 w-full rounded-xl border px-3 text-sm' style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-default)' }} /></label>{statuses.length > 0 && <label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>Status</span><AppSelect value={statuses.includes(filters.status) ? filters.status : ''} onChange={(event) => setFilter('status', event.target.value)}><option value=''>All statuses</option>{statuses.map((status) => <option key={status} value={status}>{status.replaceAll('_', ' ')}</option>)}</AppSelect></label>}{showDepartment && textFilter('department', 'Department')}{reportId === 'recruitment-applications' && <label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>Recruitment stage</span><AppSelect value={filters.recruitmentStatus} onChange={(event) => setFilter('recruitmentStatus', event.target.value)}><option value=''>All stages</option>{['APPLIED', 'SHORTLISTED', 'INTERVIEW', 'OFFERED', 'HIRED', 'REJECTED'].map((stage) => <option key={stage}>{stage}</option>)}</AppSelect></label>}{reportId === 'leave' && <label className='space-y-1.5 text-xs font-medium'><span style={{ color: 'var(--text-secondary)' }}>Leave type</span><AppSelect value={filters.leaveType} onChange={(event) => setFilter('leaveType', event.target.value)}><option value=''>All leave types</option>{['ANNUAL', 'SICK', 'CASUAL', 'UNPAID', 'OTHER'].map((type) => <option key={type}>{type}</option>)}</AppSelect></label>}{showEmployee && textFilter('employeeId', 'Employee ID')}{showProject && textFilter('projectId', 'Project ID')}{showTeam && textFilter('teamId', 'Team ID')}</div></SectionCard>;
}

function ReportKpis({ report, recordCount }: { report: FormalReportData; recordCount: number }) { return <section><SectionHeading eyebrow='Report summary' title='Key statistics' />{recordCount === 0 ? <EmptyState title='No data available' description='No records match the selected report filters.' /> : <div className='grid gap-3 sm:grid-cols-2 xl:grid-cols-4'>{report.summary.map((item) => <article key={item.label} className='rounded-2xl border p-4 shadow-sm' style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-default)' }}><p className='text-[10px] font-extrabold uppercase tracking-wide' style={{ color: 'var(--text-tertiary)' }}>{item.label}</p><p className='mt-2 text-xl font-bold' style={{ color: 'var(--text-primary)' }}>{item.value}</p></article>)}</div>}</section>; }
