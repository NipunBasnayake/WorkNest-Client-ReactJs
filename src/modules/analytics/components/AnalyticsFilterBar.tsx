...
export function AnalyticsFilterBar({ options }: { options?: AnalyticsFilterOptions }) {
  ...
  return <div className='sticky top-0 z-10 rounded-2xl border p-3 shadow-sm backdrop-blur' style={{ background: 'color-mix(in srgb, var(--bg-surface) 94%, transparent)', borderColor: 'var(--border-default)' }}>
    <div className='flex flex-wrap items-center gap-2'>
      ...
      <div className='flex rounded-xl border p-1' style={{ borderColor: 'var(--border-default)' }}>{[7, 30, 90].map((days) => <button key={days} onClick={() => setDatePreset(days)} className='rounded-lg px-2.5 py-1.5 text-xs font-semibold hover:bg-purple-500/10' style={{ color: 'var(--text-secondary)' }}>{days}d</button>)}</div>
