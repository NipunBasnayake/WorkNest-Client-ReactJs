...
interface Props { title: string; subtitle: string; data: BiChartPoint[]; variant: BiChartVariant; insight: string; series?: BiSeries[]; drillDown?: (point: BiChartPoint) => string; unit?: string; }
const palette = ['#9332ea', '#2563eb', '#10b981', '#f59e0b', '#ef4444', '#06b6d4', '#8b5cf6', '#64748b'];
...
export default function BiChartCard({ title, subtitle, data, variant, insight, series = [{ key: 'value', label: 'Value', color: '#9332ea' }], drillDown, unit = '' }: Props) {
  ...
  return <article className='overflow-hidden rounded-2xl border shadow-sm' style={{ background: 'color-mix(in srgb,var(--bg-surface) 96%,transparent)', borderColor: 'var(--border-default)' }}>
    <header className='flex flex-col gap-3 border-b p-5 sm:flex-row sm:items-start sm:justify-between' style={{ borderColor: 'var(--border-default)' }}><div><h3 className='text-sm font-bold' style={{ color: 'var(--text-primary)' }}>{title}</h3><p className='mt-1 text-xs' style={{ color: 'var(--text-secondary)' }}>{subtitle}</p></div><ReportExportMenu report={report} compact /></header>
    <div className={`h-72 p-4 ${drillDown ? 'cursor-pointer' : ''}`} onClick={() => data[0] && open(data[0])} role={drillDown ? 'link' : undefined} tabIndex={drillDown ? 0 : undefined} onKeyDown={(event) => { if (event.key === 'Enter' && data[0]) open(data[0]); }}>
    ...
    {drillDown && data.length > 0 && <div className='scrollbar-hide flex gap-1.5 overflow-x-auto px-5 pb-3 print:hidden'>{data.slice(0, 8).map((point) => <button key={`${point.id ?? ''}-${point.label}`} onClick={() => open(point)} className='inline-flex shrink-0 items-center gap-1 rounded-full border px-2 py-1 text-[10px] font-semibold hover:border-purple-400 hover:text-purple-600' style={{ borderColor: 'var(--border-default)', color: 'var(--text-secondary)' }}>{point.label}<ArrowUpRight size={10} /></button>)}</div>}
    <footer className='flex gap-2 border-t px-5 py-3 text-xs leading-5' style={{ borderColor: 'var(--border-default)', background: 'color-mix(in srgb,var(--bg-muted) 58%,transparent)', color: 'var(--text-secondary)' }}><Lightbulb size={15} className='mt-0.5 shrink-0 text-amber-500' /><span><strong style={{ color: 'var(--text-primary)' }}>Insight:</strong> {insight}</span></footer>
  </article>;
}
