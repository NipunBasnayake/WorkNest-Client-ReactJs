import { ArrowLeft, BriefcaseBusiness, CalendarDays, Clock, MapPin, Share2 } from "lucide-react";
...
export function PublicJobView({ job, tenantSlug, preview = false }: { job: PublicCareerJobDetail; tenantSlug: string; preview?: boolean }) {
  ...
  return <div className="space-y-7">
    <div className="flex flex-col gap-5 rounded-3xl border p-6 shadow-sm sm:p-8" style={{ borderColor: "var(--border-default)", background: "linear-gradient(135deg,color-mix(in srgb,var(--bg-surface) 94%,var(--color-primary-500)),var(--bg-surface))" }}>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
      <div className="space-y-6"><SectionCard><MarkdownContent>{job.description || "No job description has been provided."}</MarkdownContent></SectionCard>
        <SectionCard title="About the company"><div className="flex items-start gap-4"><div className="flex min-h-12 min-w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border p-1" style={{ borderColor: "var(--border-default)", background: "var(--bg-muted)" }}><TenantLogo size="compact" showName={false} eager /></div><div><p className="font-semibold" style={{ color: "var(--text-primary)" }}>{job.company.companyName}</p><p className="mt-1 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>{job.company.about ?? `Build your career with ${job.company.companyName}.`}</p></div></div></SectionCard>
      ...
      <aside className="space-y-4 lg:sticky lg:top-24"><SectionCard title="Role details" variant="dense"><div className="space-y-4"><Fact icon={<BriefcaseBusiness size={16} />} label="Employment" value={formatEmploymentType(job.employmentType)} /><Fact icon={<MapPin size={16} />} label="Location" value={job.location ?? "Not specified"} /><Fact icon={<Clock size={16} />} label="Experience" value={job.experience ?? "Not specified"} /><Fact icon={<CalendarDays size={16} />} label="Deadline" value={formatPublicDate(job.expiry)} /></div></SectionCard><div className="rounded-2xl p-5 shadow-lg" style={{ background: "var(--brand-action)", color: "var(--brand-on-primary)", boxShadow: "0 10px 25px var(--glow-primary)" }}><Share2 size={20} /><p className="mt-3 font-semibold">Know someone suitable?</p><p className="mt-1 text-sm opacity-85">Share this opportunity with your network.</p><div className="mt-4"><PublicShareMenu careersUrl={careersUrl} vacancyUrl={vacancyUrl} vacancyTitle={job.title} companyName={job.company.companyName} /></div></div>{applyTo ? <Button className="w-full lg:hidden" to={applyTo}>Apply Now</Button> : null}</aside>
    </div>
...
function Fact({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex items-start gap-3"><span className="mt-0.5" style={{ color: "var(--color-primary-600)" }}>{icon}</span><div><p className="text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-tertiary)" }}>{label}</p><p className="mt-0.5 text-sm font-medium" style={{ color: "var(--text-primary)" }}>{value}</p></div></div>; }
