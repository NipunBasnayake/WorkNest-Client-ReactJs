...
import type { RecruitmentApplicationEvent, RecruitmentCandidateComment, RecruitmentEmailTemplateType, RecruitmentHireFormValues, RecruitmentInterview, RecruitmentInterviewFormValues, RecruitmentStage } from "@/modules/recruitment/types";
      ...
      <div className="space-y-6">
        <SectionCard title="Candidate information" subtitle={application.referenceNumber ? `Application ${application.referenceNumber}` : undefined}>
          ...
          {application.createdBy ? (
            <div className="mt-5 flex items-center gap-2 border-t pt-4" style={{ borderColor: "var(--border-default)" }}>
              <UserAvatar name={application.createdBy.name ?? "Application owner"} email={application.createdBy.email} src={application.createdBy.avatarUrl} size="xs" />
              <p className="text-xs" style={{ color: "var(--text-secondary)" }}>Record owner: {application.createdBy.name ?? application.createdBy.email ?? "Team member"}</p>
            </div>
          ) : null}
        </SectionCard>
        ...
        <SectionCard title="Internal HR notes" subtitle="Private notes are visible only to HR and tenant administrators.">
          {!readOnly ? <form className="mb-5 flex flex-col gap-3 sm:flex-row" onSubmit={addNote}><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add a clear, useful note for the hiring record" className="min-h-20 flex-1 rounded-xl border bg-transparent px-4 py-3 text-sm outline-none focus:border-primary-500" style={{ borderColor: "var(--border-default)", color: "var(--text-primary)" }} /><Button type="submit" loading={busy} disabled={!note.trim()}><MessageSquarePlus size={15} />Add Note</Button></form> : null}
          <div className="space-y-3">{workspace.notes.isLoading ? <LoadingSkeleton lines={3} /> : (workspace.notes.data ?? []).length ? (workspace.notes.data ?? []).map((item) => <RecruitmentNote key={item.id} item={item} />) : <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>No internal notes yet.</p>}</div>
        </SectionCard>
      </div>
      ...
      <aside className="space-y-6">
        {!readOnly ? <SectionCard title="Move stage" subtitle="Only the next practical steps are available.">
          <div className="space-y-3"><AppSelect aria-label="Next application stage" value={nextStage} onChange={(event) => setNextStage(event.target.value as RecruitmentStage | "")}><option value="">Choose next stage</option>{stages.map((stage) => <option key={stage} value={stage}>{STAGE_LABELS[stage]}</option>)}</AppSelect>
            ...
            {matchingEmail(nextStage as RecruitmentStage) ? <label className="flex items-center gap-2 text-xs" style={{ color: "var(--text-secondary)" }}><input type="checkbox" checked={sendStageEmail} onChange={(event) => setSendStageEmail(event.target.checked)} className="accent-primary-600" />Send the matching candidate email</label> : null}
        ...
        <SectionCard title="Interviews">
          <div className="space-y-3">{(workspace.interviews.data ?? []).length ? (workspace.interviews.data ?? []).map((item) => <RecruitmentInterviewCard key={item.id} item={item} />) : <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>No interview scheduled.</p>}</div>
        </SectionCard>

        <SectionCard title="Application timeline">
          <ol className="space-y-4">{(workspace.timeline.data ?? []).map((item) => <RecruitmentTimelineItem key={item.id} item={item} />)}</ol>
        </SectionCard>
      </aside>
    </div>
  </div>;
...
function RecruitmentNote({ item }: { item: RecruitmentCandidateComment }) {
  const authorName = item.author?.name ?? "HR";
  return (
    <div className="rounded-xl border p-4" style={{ borderColor: "var(--border-default)", background: "var(--bg-muted)" }}>
      <p className="whitespace-pre-wrap text-sm" style={{ color: "var(--text-primary)" }}>{item.message}</p>
      <div className="mt-3 flex items-center gap-2">
        <UserAvatar name={authorName} email={item.author?.email} src={item.author?.avatarUrl} size="xs" />
        <p className="min-w-0 truncate text-xs" style={{ color: "var(--text-tertiary)" }}>{authorName} · {formatDateTime(item.createdAt)}</p>
      </div>
    </div>
  );
}

function RecruitmentInterviewCard({ item }: { item: RecruitmentInterview }) {
  const interviewerName = item.interviewer?.name ?? "Interviewer not assigned";
  return (
    <div className="rounded-xl border p-3" style={{ borderColor: "var(--border-default)" }}>
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{formatDateTime(item.scheduledAt)}</p>
        <RecruitmentStatusBadge value={item.status} />
      </div>
      <p className="mt-1 text-xs" style={{ color: "var(--text-secondary)" }}>{item.mode === "REMOTE" ? "Online" : item.mode === "ONSITE" ? "Physical" : "Phone"} · {item.meetingLink || item.location || "Details pending"}</p>
      <div className="mt-3 flex items-center gap-2">
        <UserAvatar name={interviewerName} email={item.interviewer?.email} src={item.interviewer?.avatarUrl} size="xs" />
        <span className="min-w-0 truncate text-xs" style={{ color: "var(--text-secondary)" }}>{interviewerName}</span>
      </div>
      {item.feedback?.reviewer ? (
        <div className="mt-3 flex items-center gap-2 border-t pt-3" style={{ borderColor: "var(--border-default)" }}>
          <UserAvatar name={item.feedback.reviewer.name ?? "Reviewer"} email={item.feedback.reviewer.email} src={item.feedback.reviewer.avatarUrl} size="xs" />
          <span className="min-w-0 truncate text-xs" style={{ color: "var(--text-secondary)" }}>Feedback by {item.feedback.reviewer.name ?? "Reviewer"}{item.feedback.rating ? ` · ${item.feedback.rating}/5` : ""}</span>
        </div>
      ) : null}
    </div>
  );
...
function RecruitmentTimelineItem({ item }: { item: RecruitmentApplicationEvent }) {
  return (
    <li className="relative border-l-2 border-primary-200 pl-4">
      <span className="absolute -left-[5px] top-1.5 h-2 w-2 rounded-full bg-primary-500" />
      <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{item.title}</p>
      {item.detail ? <p className="mt-0.5 text-xs" style={{ color: "var(--text-secondary)" }}>{item.detail}</p> : null}
      <div className="mt-2 flex items-center gap-2">
        {item.actor ? <UserAvatar name={item.actor.name ?? "Team member"} email={item.actor.email} src={item.actor.avatarUrl} size="xs" /> : null}
        <p className="min-w-0 truncate text-[11px]" style={{ color: "var(--text-tertiary)" }}>{formatDateTime(item.occurredAt)}{item.actor?.name ? ` · ${item.actor.name}` : ""}</p>
      </div>
    </li>
  );
}
...
function Info({ label, value, href }: { label: string; value?: string; href?: string }) { return <div><dt className="text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-tertiary)" }}>{label}</dt><dd className="mt-1 break-words text-sm" style={{ color: "var(--text-primary)" }}>{value ? href ? <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="text-primary-600 hover:underline">{value}</a> : value : "—"}</dd></div>; }
