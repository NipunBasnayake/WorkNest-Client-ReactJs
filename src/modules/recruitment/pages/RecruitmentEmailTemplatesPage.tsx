  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
      <SectionCard title="Templates" subtitle="Seven messages cover the full hiring flow.">
        <nav className="space-y-1" aria-label="Recruitment email templates">{(templatesQuery.data ?? []).map((template) => <button key={template.type} type="button" onClick={() => setSelectedType(template.type)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium transition ${selectedType === template.type ? "bg-primary-600 text-white shadow-sm" : "hover:bg-primary-500/5"}`} style={selectedType === template.type ? undefined : { color: "var(--text-secondary)" }}><Mail size={16} />{LABELS[template.type]}</button>)}</nav>
      </SectionCard>
      {draft ? <div className="space-y-6">
        <SectionCard title={LABELS[draft.type]} subtitle="Variables are replaced automatically when the message is sent.">
          <div className="space-y-5"><Input id="template-subject" label="Subject" value={draft.subject} onChange={(event) => setDraft((current) => current ? { ...current, subject: event.target.value } : current)} required />
            <div><label htmlFor="template-body" className="mb-1.5 block text-sm font-medium" style={{ color: "var(--text-secondary)" }}>Message (Markdown)</label><textarea id="template-body" value={draft.bodyMarkdown} onChange={(event) => setDraft((current) => current ? { ...current, bodyMarkdown: event.target.value } : current)} className="min-h-72 w-full rounded-2xl border bg-transparent p-4 font-mono text-sm leading-6 outline-none focus:border-primary-500" style={{ borderColor: "var(--border-default)", color: "var(--text-primary)" }} /></div>
            <div><p className="mb-2 text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-tertiary)" }}>Available variables</p><div className="flex flex-wrap gap-2">{draft.availableVariables.map((variable) => <button key={variable} type="button" onClick={() => setDraft((current) => current ? { ...current, bodyMarkdown: `${current.bodyMarkdown}${current.bodyMarkdown.endsWith(" ") ? "" : " "}{{${variable}}}` } : current)} className="rounded-lg border px-2.5 py-1.5 font-mono text-xs text-primary-600 hover:bg-primary-500/5" style={{ borderColor: "var(--border-default)" }}>{`{{${variable}}}`}</button>)}</div></div>
            <label className="flex items-center gap-2 text-sm" style={{ color: "var(--text-secondary)" }}><input type="checkbox" checked={draft.enabled} onChange={(event) => setDraft((current) => current ? { ...current, enabled: event.target.checked } : current)} className="accent-primary-600" />Template enabled</label>
          </div>
        </SectionCard>
