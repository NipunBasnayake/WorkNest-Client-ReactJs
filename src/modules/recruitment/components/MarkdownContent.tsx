...
const components: Components = {
  ...
  a: ({ href, children }) => <a href={href} target="_blank" rel="noreferrer" className="font-medium text-primary-600 underline decoration-primary-300 underline-offset-4 hover:text-primary-700">{children}</a>,
  ...
  blockquote: ({ children }) => <blockquote className="my-5 border-l-4 border-primary-400 bg-primary-500/5 px-5 py-1 italic" style={{ color: "var(--text-secondary)" }}>{children}</blockquote>,
  ...
  code: ({ children, className }) => className
    ...
    : <code className="rounded bg-primary-500/10 px-1.5 py-0.5 text-[0.9em] text-primary-700 dark:text-primary-300">{children}</code>,
  ...
  input: ({ type, checked, ...props }) => <input {...props} type={type} checked={checked} readOnly className="mr-2 accent-primary-600" />,
};
